...
void collectTime()
{
  ...
  data += String(GPS_hour) + ",";

  if(GPS_minutes < 10)
  {
    data += "0";
  }
  data += String(GPS_minutes) + ",";

  if(GPS_seconds < 10)
  {
    data += "0";
  }
  data += String(GPS_seconds);

  writeSD(METAFILE, data);
...
bool initSD()
{
  return sd.begin(SD_CS_PIN, SD_SCK_MHZ(50));
}
...
void logData() {
  ...
  while (1) {
    ...
    if (got_a_key) {
      the_key = customKeypad.key[0].kchar;
    }
